...
class RuleEngine {
  constructor() {
    ...
    this.loadRules()
    this.loadSymptomRelations()
  }

  async loadRules() {
    try {
      this.rules = await Rule.find({ active: true }).sort({ priority: -1 })
  ...
  async loadSymptomRelations() {
    try {
      this.symptomRelations = await SymptomRelations.find({ isActive: true })
      console.log(`Loaded ${this.symptomRelations.length} symptom relations`)
    } catch (error) {
      console.error('Error loading symptom relations:', error)
    }
  }

  async evaluate(userProfile, symptoms, duration_days, vitals = {}, notes = '') {
    // Reload rules to ensure we have the latest
    await this.loadRules()
    await this.loadSymptomRelations()
    ...
    const redFlagRules = this.rules.filter(rule => 
      ...
      if (match.matched) {
        ...
        if (rule.action.type === 'emergency') {
          return {
            ...
            suggested_additional: await this.getProbableSymptoms(symptoms)
          }
        }
      }
    }
    ...
    const otherRules = this.rules.filter(rule => 
      !rule.tags.includes('redflag') || !rule.tags.includes('emergency')
    )
    ...
    for (const [actionType, group] of Object.entries(actionGroups)) {
      ...
      suggestions.push({
        type: actionType,
        message: group.rules[0].action.message, // Use first rule's message
    ...
    suggestions.sort((a, b) => {
      ...
      // If confidence is equal, sort by priority (emergency > see_gp > teleconsult > self_care)
  ...
  buildFacts(userProfile, symptoms, duration_days, vitals, notes) {
    ...
    // Convert symptoms array to object for easier lookup
  ...
  computeRuleConfidence(rule, matchedFacts, facts) {
    ...
    // Base confidence on rule weight
    confidence *= rule.weight

    // Adjust based on number of matched facts vs total conditions
    ...
    // Red-flag rules get higher confidence
  ...
  async getProbableSymptoms(symptoms) {
    if (!symptoms || symptoms.length === 0) return []

    const symptomIds = symptoms.map(s => s.id)
    const relations = this.symptomRelations.filter(rel => 
    ...
    for (const relation of relations) {
      for (const target of relation.targets) {
        if (!symptomIds.includes(target.id)) {
          ...
          // Use independence assumption: P(A|B,C) = 1 - (1-P(A|B)) * (1-P(A|C))
    ...
    return Array.from(candidateSymptoms.entries())
      ...
      .filter(item => item.probability > 0.3) // Only show if >30% likely
  ...
  async testRule(payload) {
    ...
    return await this.evaluate(userProfile, symptoms, duration_days, vitals, notes)
  }
}
